...
def scrape (Year):
    ...
    url = str('http://stats.espncricinfo.com/ci/engine/stats/index.html?class=2;filter=advanced;groupby=overall;orderby=runs;spanmax1=31+Dec+' + str(Year+5) + ';spanmin1=01+Jan+' +str(Year-5) +';spanval1=span;template=results;type=bowling')
    r = requests.get(url)
    print (url)
    data = r.content
...
for i in range (1971 , CurrentYear+1):
    scrape (i)
