# Scrape data from cricinfo using bs4
...
def scrape (x):
    ...
    TestBatData = soup.find(text='ODIs')
    ...
    TestBowlData = soup.findAll(text='ODIs')
 ...
 #           ReducedStats.append(TestBowlStats[i])
    ...
    try:
        FirstTest = soup.find(text='ODI debut').parent.parent.parent.contents[3].contents[0][-5:-1]
    ...
    try:
        LastTest = soup.find(text='Last ODI').parent.parent.parent.contents[3].contents[0][-5:-1]
...
def guruscrape (x):
    global ReducedStats
    url = str('http://stats.espncricinfo.com/ci/engine/player/' + str(x) + '.html?class=2;template=results;type=batting')
    r = requests.get(url)
    player = r.content
    soup = BeautifulSoup(player, 'html.parser')
...
def PlayerList (x, y):
    ...
    url = str(str('http://www.espncricinfo.com/ci/content/player/caps.html?country=') +str(y)+ str(';class=2'))
    r = requests.get(url)
    player = r.content
    newsoup = BeautifulSoup(player, 'html.parser')
    Format = ('Name, BowlStyle, ODIs, Runs, BallsFaced WHERE POSSIBLE, TimesOut, Balls, Runs, Wickets, FirstYear, LastYear, FCGames, FCRuns, FCBatAve, FCWickets, FCBowlAve, GamesCaptained, GamesWK, InnsOpen')
