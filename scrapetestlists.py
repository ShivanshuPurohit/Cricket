...
def gamescrape (Year):
    url = str('http://stats.espncricinfo.com/ci/engine/records/team/match_results.html?class=2;id='+str(Year)+';type=year')
    r = requests.get(url)
    #print (url)
    data = r.content
...
g = open('odimatchlist.txt', 'w')
g.close()

for x in range (1971, CurrentYear+1):
    y = 0
    try:
        games = gamescrape(x)
        y = len(games)
